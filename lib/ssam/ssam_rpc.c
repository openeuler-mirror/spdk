...
uint8_t g_hpd_delete_session_times[2000] = {0};
...
{
	if (gfunc_id <= SSAM_PF_MAX_NUM && (ssam_get_en_hpd() == false)) {
	...
	if (req->vqueue > SPDK_SSAM_BLK_MAX_VQ_SIZE || req->vqueue == 0) {
			       ...
			       const struct spdk_json_val *params)
{
	...
	if (ssam_get_en_hpd() == true) {
		bdev = spdk_bdev_get_by_name(req.dev_name);
		if (bdev == NULL) {
...
static const struct spdk_json_object_decoder g_rpc_delete_ssam_ctrlr_decoder[] = {
    ...
    {"force", offsetof(struct rpc_delete_ssam_ctrlr, force), spdk_json_decode_bool},
};
			   ...
			   const struct spdk_json_val *params)
{
    ...
    g_delete_flag = 0;
    g_hpd_delete_session_times[gfunc_id] = 0;

    smdev = ssam_dev_next(NULL);
    if ((smdev->type == VIRTIO_TYPE_BLK) && (req.force != true)) {
        while (smdev != NULL) {
	...
	while (smdev != NULL) {
		...
		rc = ssam_session_unregister(smsession, req.force);
		if (rc != 0) {
			/*
...
static void
rpc_ssam_hot_upgrade_start_io_poll(struct spdk_jsonrpc_request *request,
			  const struct spdk_json_val *params)
{
	int rc = 0;
	int status = spdk_ssam_get_hot_upgrade_status();

	if (status == SSAM_HOT_UPGRADE_INIT_DONE) {
		rc = ssam_check_device_status(SSAM_HOT_SWAP_PROCESS);
		if (rc != 0) {
			SPDK_ERRLOG("ssam hot upgrade start io poll failed: device check failed \n");
			spdk_jsonrpc_send_error_response(request, SPDK_JSONRPC_ERROR_INTERNAL_ERROR,
							 spdk_strerror(-rc));
			return;
		}
		...
		if (rc != 0) {
			SPDK_ERRLOG("ssam hot upgrade start io poll failed: io resend error \n");
			spdk_jsonrpc_send_error_response(request, SPDK_JSONRPC_ERROR_INTERNAL_ERROR,
							 spdk_strerror(-rc));
			return;
		} 
		spdk_ssam_set_hot_upgrade_status(SSAM_HOT_UPGRADE_DONE);
		SPDK_NOTICELOG("ssam hot upgrade start io poll success \n");
		spdk_jsonrpc_send_bool_response(request, true);
		return;
	}

	SPDK_ERRLOG("ssam hot upgrade status error, status :%d", status);
    spdk_jsonrpc_send_error_response(request, SPDK_JSONRPC_ERROR_INTERNAL_ERROR,
	 						"Hot upgrade status error");
	return;
}

SPDK_RPC_REGISTER("hot_upgrade_start_io_poll", rpc_ssam_hot_upgrade_start_io_poll, SPDK_RPC_RUNTIME)

static void
rpc_ssam_check_hot_upgrade_status(struct spdk_jsonrpc_request *request,
			  const struct spdk_json_val *params)
{
	int status = spdk_ssam_get_hot_upgrade_status();

	if (status == SSAM_HOT_UPGRADE_INIT_DONE) {
		SPDK_NOTICELOG("ssam hot upgrade init done, upgrade status :%d\n", status);
		spdk_jsonrpc_send_bool_response(request, true);
		return;
	}

	SPDK_ERRLOG("ssam hot upgrade not init done, upgrade status :%d\n", status);
    spdk_jsonrpc_send_bool_response(request, false);

	return;
}

SPDK_RPC_REGISTER("check_hot_upgrade_status", rpc_ssam_check_hot_upgrade_status, SPDK_RPC_RUNTIME)
