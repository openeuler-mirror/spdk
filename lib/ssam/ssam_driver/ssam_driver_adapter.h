...
struct ssam_drv_ops {
	...
	int (*hvio_qos_get_group_id_by_func)(uint32_t glb_function_id, uint32_t *group_id);
};
...
int ssam_drv_hotplug_del_async(uint16_t port_id);
bool ssam_drv_hotplug_enable_check(void);
int ssam_drv_hotplug_del_async_check(uint16_t port_id);
int ssam_drv_virtio_vq_bind_core(uint16_t glb_function_id, uint16_t queue_num);
int ssam_drv_virtio_vq_unbind_core(uint16_t glb_function_id);
int ssam_drv_qos_set_limit(uint32_t level, uint32_t id, struct hivo_qos_cfg *qos_cfg);
int ssam_drv_qos_get_limit(uint32_t level, uint32_t id, struct hivo_qos_cfg *qos_cfg);
int ssam_drv_qos_group_func_mmap(uint32_t group_id, uint32_t func_id);
int ssam_drv_qos_group_func_unmmap(uint32_t group_id, uint32_t func_id);
int ssam_drv_qos_get_func_of_group(uint32_t group_id, struct storage_qos_func_list *result);
int ssam_drv_qos_clear(void);
int ssam_drv_qos_get_group_id(uint32_t glb_function_id, uint32_t *group_id);
