...
#define QOS_MAX_FUNCTION_NUM_PER_GROUP 64
...
struct ssam_qos_cfg {
	/* qos type: 0-pps, 1-bps, 2-bps&pps, other-invalid */
	uint32_t qos_type;
	/* C bucket config */
	uint32_t pps_cir;
	uint32_t pps_cbs;
...
struct ssam_qos_func_list {
	/* function number */
	uint32_t func_num;
	/* max function num per QoS group */
	uint16_t func_list[QOS_MAX_FUNCTION_NUM_PER_GROUP];
};

struct ssam_qos_group_func {
	struct  ssam_qos_func_list func_info;
	uint32_t group_id;
};
/**
 ...
 * \return 0 for succeed or not 0 for failed
 */
int ssam_virtio_vq_bind_core(uint16_t glb_function_id, uint16_t queue_num);
 ...
 * \return 0 for succeed or not 0 for failed
 */
int ssam_virtio_vq_unbind_core(uint16_t glb_function_id);
 ...
 *   - others: fail, refer to errno.h
 */
int ssam_hotplug_del_async(uint16_t port_id);
 ...
 *   - 2:  try again
 */
int ssam_hotplug_del_async_check(uint16_t port_id);

/**
 * @brief set limit of QoS.
 ...
 */
int ssam_qos_set_limit(uint32_t level, uint32_t id, struct ssam_qos_cfg *qos_cfg);

/**
 * @brief get limit of QoS.
 * @param level  level of QoS limit. 0-host, 1-group, 2-function
 * @param id  index of target, host_id/group_id/func_id
 * @param[out] qos_cfg  output for QoS limit.
 * @return
 *   - 0: success
 *   - other: fail, refer to errno.h
 */
int ssam_qos_get_limit(uint32_t level, uint32_t id, struct ssam_qos_cfg *qos_cfg);
 ...
 *   - other: fail, refer to errno.h
 */
int ssam_qos_group_func_mmap(uint32_t group_id, uint32_t func_id);
 ...
 *   - other: fail, refer to errno.h
 */
int ssam_qos_group_func_unmmap(uint32_t group_id, uint32_t func_id);
 ...
 *   - other: fail, refer to errno.h
 */
int ssam_qos_get_func_of_group(uint32_t group_id, struct ssam_qos_func_list *result);
 ...
 *   - 0: success
 */
int ssam_qos_clear(void);

/**
 * @brief get group id by function id
 * @param func_id index of func.
 * @param group_id index of group, 0 means invalid
 * @return
 *   - 0: get group id success but need to check group_id
 *   - others: fail, refer to errno.h
 */
int ssam_qos_get_group_id(uint32_t function_id, uint32_t *group_id);
