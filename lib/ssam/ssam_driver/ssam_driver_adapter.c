...
static struct ssam_drv_ops_map g_ssam_drv_ops_map[] = {
	...
	SSAM_DRV_ADD_FUNC(g_ssam_drv_ops, hvio_hotplug_cfg),
	SSAM_DRV_ADD_FUNC(g_ssam_drv_ops, hvio_hotplug_add),
	SSAM_DRV_ADD_FUNC(g_ssam_drv_ops, hvio_hotplug_del),
	SSAM_DRV_ADD_FUNC(g_ssam_drv_ops, hvio_hotplug_del_async),
	SSAM_DRV_ADD_FUNC(g_ssam_drv_ops, hvio_hotplug_enable_check),
	SSAM_DRV_ADD_FUNC(g_ssam_drv_ops, hvio_hotplug_del_async_check),
	...
	SSAM_DRV_ADD_FUNC(g_ssam_drv_ops, hvio_qos_set_limit),
	SSAM_DRV_ADD_FUNC(g_ssam_drv_ops, hvio_qos_get_limit),
	SSAM_DRV_ADD_FUNC(g_ssam_drv_ops, hvio_qos_group_func_mmap),
	SSAM_DRV_ADD_FUNC(g_ssam_drv_ops, hvio_qos_group_func_unmmap),
	SSAM_DRV_ADD_FUNC(g_ssam_drv_ops, hvio_qos_get_func_of_group),
	SSAM_DRV_ADD_FUNC(g_ssam_drv_ops, hvio_qos_clear),
	SSAM_DRV_ADD_FUNC(g_ssam_drv_ops, hvio_qos_get_group_id_by_func),
};
...
int
ssam_drv_hotplug_cfg(void)
{
	struct ssam_drv_ops *ops = NULL;

	ops = ssam_get_drv_ops();
	SSAM_FUNC_PTR_OR_ERR_RET(ops->hvio_hotplug_cfg, SSAM_DRV_FUNC_NO_PTR);

	ops->hvio_hotplug_cfg();
	return 0;
}

int
ssam_drv_hotplug_add(uint16_t port_id)
{
	int ret;
	struct ssam_drv_ops *ops = NULL;

	ops = ssam_get_drv_ops();
	SSAM_FUNC_PTR_OR_ERR_RET(ops->hvio_hotplug_add, SSAM_DRV_FUNC_NO_PTR);

	ret = ops->hvio_hotplug_add(port_id);
	if (ret != 0) {
		SPDK_ERRLOG("hvio_hotplug_add exec fail, ret=%d", ret);
		return ret;
	}
...
int
ssam_drv_hotplug_del(uint16_t port_id)
{
	int ret;
	struct ssam_drv_ops *ops = NULL;

	ops = ssam_get_drv_ops();
	SSAM_FUNC_PTR_OR_ERR_RET(ops->hvio_hotplug_del, SSAM_DRV_FUNC_NO_PTR);

	ret = ops->hvio_hotplug_del(port_id);
	if (ret != 0) {
		SPDK_ERRLOG("hvio_hotplug_del exec fail, ret=%d", ret);
		return ret;
	}
...
int
ssam_drv_hotplug_del_async(uint16_t port_id)
{
	int ret;
	struct ssam_drv_ops *ops = NULL;

	ops = ssam_get_drv_ops();
	SSAM_FUNC_PTR_OR_ERR_RET(ops->hvio_hotplug_del_async, SSAM_DRV_FUNC_NO_PTR);

	ret = ops->hvio_hotplug_del_async(port_id);
	if (ret != 0) {
		SPDK_ERRLOG("hvio_hotplug_del_async exec fail, ret=%d", ret);
		return ret;
	}
...
bool
ssam_drv_hotplug_enable_check(void)
{
	bool ret;
	struct ssam_drv_ops *ops = NULL;

	ops = ssam_get_drv_ops();
	SSAM_FUNC_PTR_OR_ERR_RET(ops->hvio_hotplug_enable_check, SSAM_DRV_FUNC_NO_PTR);

	ret = ops->hvio_hotplug_enable_check();
	SPDK_NOTICELOG("hvio_hotplug_enable_check status ret=%d\n", ret);

	return ret;
}

int
ssam_drv_hotplug_del_async_check(uint16_t port_id)
{
	int ret;
	struct ssam_drv_ops *ops = NULL;

	ops = ssam_get_drv_ops();
	SSAM_FUNC_PTR_OR_ERR_RET(ops->hvio_hotplug_del_async_check, SSAM_DRV_FUNC_NO_PTR);

	ret = ops->hvio_hotplug_del_async_check(port_id);
	if (ret != 0) {
		SPDK_ERRLOG("hvio_hotplug_del_async_check exec fail, ret=%d", ret);
		return ret;
	}
...
int
ssam_drv_qos_set_limit(uint32_t level, uint32_t id, struct hivo_qos_cfg *qos_cfg)
{
	int ret;
	struct ssam_drv_ops *ops = NULL;

	SPDK_INFOLOG(ssam, "set qos limit, level=%u, id=%u, type=%u,"
				"pps_cir=%u, pps_cbs=%u, "
				"bps_cir=%u, bps_cbs=%u, "
				"pps_pir=%u, pps_pbs=%u, "
				"bps_pir=%u, bps_pbs=%u\n",
				level, id, qos_cfg->qos_type,
				qos_cfg->pps_cir, qos_cfg->pps_cbs,
				qos_cfg->bps_cir, qos_cfg->bps_cbs,
				qos_cfg->pps_pir, qos_cfg->pps_pbs,
				qos_cfg->bps_pir, qos_cfg->bps_pbs);

	ops = ssam_get_drv_ops();
	SSAM_FUNC_PTR_OR_ERR_RET(ops->hvio_qos_set_limit, SSAM_DRV_FUNC_NO_PTR);

	ret = ops->hvio_qos_set_limit(level, id, qos_cfg);
	if (ret != 0) {
		SPDK_ERRLOG("hvio_qos_set_limit exec fail, ret %d\n", ret);
	}
...
int
ssam_drv_qos_get_limit(uint32_t level, uint32_t id, struct hivo_qos_cfg *qos_cfg)
{
	int ret;
	struct ssam_drv_ops *ops = NULL;

	ops = ssam_get_drv_ops();
	SSAM_FUNC_PTR_OR_ERR_RET(ops->hvio_qos_get_limit, SSAM_DRV_FUNC_NO_PTR);

	ret = ops->hvio_qos_get_limit(level, id, qos_cfg);
	if (ret != 0) {
		SPDK_ERRLOG("hvio_qos_get_limit exec fail, level=%u, id=%u, ret=%d\n", level, id, ret);
	}
...
int
ssam_drv_qos_group_func_mmap(uint32_t group_id, uint32_t func_id)
{
	int ret;
	struct ssam_drv_ops *ops = NULL;

	ops = ssam_get_drv_ops();
	SSAM_FUNC_PTR_OR_ERR_RET(ops->hvio_qos_group_func_mmap, SSAM_DRV_FUNC_NO_PTR);

	ret = ops->hvio_qos_group_func_mmap(group_id, func_id);
	if (ret != 0) {
		SPDK_ERRLOG("hvio_qos_group_func_mmap exec fail, group_id=%u, func_id=%u, ret=%d\n", group_id, func_id, ret);
	}
...
int
ssam_drv_qos_group_func_unmmap(uint32_t group_id, uint32_t func_id)
{
	int ret;
	struct ssam_drv_ops *ops = NULL;

	ops = ssam_get_drv_ops();
	SSAM_FUNC_PTR_OR_ERR_RET(ops->hvio_qos_group_func_unmmap, SSAM_DRV_FUNC_NO_PTR);

	ret = ops->hvio_qos_group_func_unmmap(group_id, func_id);
	if (ret != 0) {
		SPDK_ERRLOG("hvio_qos_group_func_unmmap exec fail, group_id=%u, func_id=%u, ret=%d\n", group_id, func_id, ret);
	}
...
int
ssam_drv_qos_get_func_of_group(uint32_t group_id, struct storage_qos_func_list *result)
{
	int ret;
	struct ssam_drv_ops *ops = NULL;

	ops = ssam_get_drv_ops();
	SSAM_FUNC_PTR_OR_ERR_RET(ops->hvio_qos_get_func_of_group, SSAM_DRV_FUNC_NO_PTR);

	ret = ops->hvio_qos_get_func_of_group(group_id, result);
	if (ret != 0) {
		SPDK_ERRLOG("hvio_qos_get_func_of_group exec fail, group_id=%u, ret=%d", group_id, ret);
	}
...
int
ssam_drv_qos_clear(void)
{
	int ret;
	struct ssam_drv_ops *ops = NULL;

	ops = ssam_get_drv_ops();
	SSAM_FUNC_PTR_OR_ERR_RET(ops->hvio_qos_clear, SSAM_DRV_FUNC_NO_PTR);

	ret = ops->hvio_qos_clear();
	if (ret != 0) {
		SPDK_ERRLOG("hvio_qos_clear failed\n");
	}
...
int
ssam_drv_qos_get_group_id(uint32_t function_id, uint32_t *group_id)
{
	int ret = 0;
	struct ssam_drv_ops *ops = NULL;

	ops = ssam_get_drv_ops();
	SSAM_FUNC_PTR_OR_ERR_RET(ops->hvio_qos_get_group_id_by_func, SSAM_DRV_FUNC_NO_PTR);

	ret = ops->hvio_qos_get_group_id_by_func(function_id, group_id);
	if (ret != 0) {
		SPDK_ERRLOG("hvio get group id(%u) failed, ret=%d\n", *group_id, ret);
	}
