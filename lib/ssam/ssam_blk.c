...
ssam_blk_stop_cpl_cb(struct spdk_ssam_session *smsession, void **ctx)
{
	...
	if (delete_dev_times[smsession->gfunc_id] == ssam_get_core_num()) {
		...
		if (smsession->gfunc_id > SSAM_PF_MAX_NUM) {
			   ...
			   struct spdk_json_write_ctx *w)
{
	...
	if (bsmsession->smsession.gfunc_id > SSAM_PF_MAX_NUM) {
...
{
	struct spdk_ssam_blk_session *bsmsession = ssam_to_blk_session(smsession);
	gfunc_session_number++;

	struct spdk_ssam_send_event_flag send_event_flag = {
		.need_async = true,
		.need_rsp = (gfunc_session_number == ssam_get_core_num()),
	};
	if (send_event_flag.need_rsp == false) {
...
ssam_blk_stop(struct spdk_ssam_session *smsession)
{
	...
	struct spdk_ssam_send_event_flag send_event_flag = {
		.need_async = true,
		.need_rsp = (delete_flag == ssam_get_core_num()),
	};
	if (send_event_flag.need_rsp == false) {
		smsession->rsp_fn = NULL;
		smsession->rsp_ctx = NULL;
