...
#define HPD_DEL_POLLER_PERIOD       (1000 * 1000)
...
#define SSAM_IO_PROCESS_TIME_OUT    2
...
static int g_gfunc_session_number = 0;

bool g_hpd_to_async[2000] = {0};
static uint8_t g_blk_set_times[2000] = {0};
static bool g_hpd_del_flag[2000] = {0};
static bool g_hpd_add_fail_flag[2000] = {0};

enum ssam_hpd_del {
	SSAM_HPD_RM_SUC,
	SSAM_HPD_DEL_REM_FAIL,
	SSAM_HPD_INVAL_ID,
	SSAM_HPD_RM_REPEAT,
};

enum ssam_hpd_del_check {
	SSAM_HPD_CHECK_SUC,
	SSAM_HPD_RM_FAIL,
	SSAM_HPD_RETRY,
};
...
struct spdk_ssam_blk_session {
	...
	struct spdk_poller *stop_session_poller;
	struct spdk_poller *hpd_stop_poller;

	/* controller statistics. */
	struct ssam_blk_stat blk_stat;

	/* accumulated I/O statistics */
	struct spdk_bdev_io_stat vq_stat[SPDK_SSAM_MAX_VQUEUES];

	/* controller vq statistics. */
	struct ssam_blk_stat vq_blk_stat[SPDK_SSAM_MAX_VQUEUES];
...
static void ssam_blk_roback(struct spdk_ssam_session_reg_info *info, const char *dev_name,
			    bool readonly, char *serial);
static void ssam_blk_del_recover(struct spdk_ssam_session *smsession,
				 struct spdk_bdev *bdev_roback);
static int ssam_destroy_session_cb(void *arg);
...
static void
ssam_hpd_del_release_resource(void *arg)
{
    ...
    struct spdk_bdev *bdev_roback = bsmsession->bdev;
    ...
    if (ssam_get_hash_mode() == SSAM_VQ_HASH_MODE) {
        rc = ssam_virtio_vq_unbind_core(smsession->gfunc_id);
        if (rc != 0) {
            SPDK_WARNLOG("%s: virtio blk vq unbind core failed.\n", smsession->name);
        }
    }

    if ((smsession->gfunc_id > SSAM_PF_MAX_NUM) || (ssam_get_en_hpd() == true)) {
        rc = ssam_virtio_blk_release_resource(smsession->gfunc_id);
        if (rc != 0) {
            SPDK_WARNLOG("%s: virtio blk release vq failed.\n", smsession->name);
        }
    }

    SPDK_NOTICELOG("BLK controller %s deleted\n", smsession->name);
    spdk_poller_unregister(&bsmsession->hpd_stop_poller);
...
static int
ssam_hpd_del_poller_cb(void *arg)
{
    struct spdk_ssam_blk_session *bsmsession = (struct spdk_ssam_blk_session *)arg;
    struct spdk_ssam_session *smsession = &bsmsession->smsession;
    spdk_ssam_session_rsp_fn rsp_fn = smsession->rsp_fn;
    ...
    rc = ssam_hotplug_del_async_check(smsession->gfunc_id);
    if (rc == SSAM_HPD_CHECK_SUC) {
        ssam_hpd_del_release_resource(smsession);
        ssam_unlock();
        return SPDK_POLLER_BUSY;
    } else if (rc == SSAM_HPD_RETRY) {
        bsmsession->hot_plug_poller_number++;
        if (bsmsession->hot_plug_poller_number >= HPD_DEL_POLLER_NUM) {
            SPDK_ERRLOG("BLK controller %s hotplug deleted failed\n", smsession->name);
            spdk_poller_unregister(&bsmsession->hpd_stop_poller);
...
static 
void ssam_free_session(struct spdk_ssam_session *smsession)
{
	struct spdk_ssam_blk_session *bsmsession = ssam_to_blk_session(smsession);
	if (smsession->name != NULL) {
		free(smsession->name);
...
ssam_blk_stop_cpl_cb(struct spdk_ssam_session *smsession, void **ctx)
{
	...
	int rc = -1;
	struct spdk_bdev *bdev_roback = bsmsession->bdev;
	bsmsession->hpd_stop_poller = NULL;

	g_hpd_delete_session_times[smsession->gfunc_id]++;
	ssam_dev_bdev_remove_cpl_cb(smsession, NULL);

	if (g_hpd_delete_session_times[smsession->gfunc_id] == ssam_get_core_num()) {

		int hot_upgrade_status = spdk_ssam_get_hot_upgrade_status();
		if (hot_upgrade_status != SSAM_HOT_UPGRADE_DONE) {
			ssam_free_session(smsession);
			SPDK_NOTICELOG("BLK controller %s deleted, hot upgrade status:%d \n", smsession->name, hot_upgrade_status);
			return;
		}

        if (ssam_get_en_hpd() == true && g_hpd_add_fail_flag[smsession->gfunc_id] == false) {
            rc = ssam_hotplug_del_async(smsession->gfunc_id);
            if (rc != SSAM_HPD_RM_SUC && rc != SSAM_HPD_RM_REPEAT) {
                if (rc == SSAM_HPD_DEL_REM_FAIL) {
                    SPDK_ERRLOG("BLK controller %s hotplug deleted failed\n", smsession->name);
                }
                if (rc == SSAM_HPD_INVAL_ID) {
                    SPDK_ERRLOG("the func_id %d is invalid\n", smsession->gfunc_id);
                }
                ssam_blk_del_recover(smsession, bdev_roback);
                goto out;
            }

            bsmsession->bdev = bdev_roback;
            bsmsession->hot_plug_poller_number = 0;
            bsmsession->hpd_stop_poller = SPDK_POLLER_REGISTER(ssam_hpd_del_poller_cb,
            	smsession, HPD_DEL_POLLER_PERIOD);
            return;
        }
		...
		SPDK_NOTICELOG("clear QoS cfg\n");
		rc = ssam_qos_set_limit(SSAM_QOS_LEVEL_FUNCTION, smsession->gfunc_id, &cfg);
		if (rc != 0) {
			SPDK_WARNLOG("clean func id: %u failed.\n", smsession->gfunc_id);
		}
		rc = ssam_qos_get_group_id(smsession->gfunc_id, &group_id);
		/* remove func_id from qos group when delete func */
		if (rc == 0 && group_id != 0) {
			rc = ssam_qos_group_func_unmmap(group_id, smsession->gfunc_id);
			if (rc != 0) {
				SPDK_WARNLOG("remove func id:%u from group_id(%u) failed.\n", smsession->gfunc_id, group_id);
			}
		}
...
static void
ssam_blk_stat_statistics(struct spdk_ssam_blk_task *task, struct spdk_bdev_io_stat *stat,
			 struct ssam_blk_stat *blk_stat, uint8_t status)
{
#ifdef PERF_STAT
	...
	if (req->type == VIRTIO_BLK_T_IN) {   /* read */
		stat->read_latency_ticks += total_tsc;
		stat->bytes_read += task->payload_size;
		stat->num_read_ops++;
		if (status == VIRTIO_BLK_S_OK) {
			blk_stat->complete_read_ios++;
		} else {
			blk_stat->err_read_ios++;
		}
	} else if (req->type == VIRTIO_BLK_T_OUT) {    /* write */
		stat->write_latency_ticks += total_tsc;
		stat->bytes_written += task->payload_size;
		stat->num_write_ops++;
		if (status == VIRTIO_BLK_S_OK) {
			blk_stat->complete_write_ios++;
		} else {
			blk_stat->err_write_ios++;
		}
		...
		if (status == VIRTIO_BLK_S_OK) {
			...
			blk_stat->err_flush_ios++;
		}
	} else {
		blk_stat->other_ios++;
		if (status == VIRTIO_BLK_S_OK) {
			blk_stat->complete_other_ios++;
		} else {
			blk_stat->err_other_ios++;
		}
	}

	blk_stat->dma_tsc += dma_tsc;
	blk_stat->bdev_tsc += bdev_tsc;
	blk_stat->bdev_submit_tsc += bdev_submit_tsc;
	blk_stat->complete_tsc += complete_tsc;
	blk_stat->total_tsc += total_tsc;
	blk_stat->internel_tsc += total_tsc - complete_tsc - bdev_tsc - dma_tsc;
	blk_stat->count += 1;
#endif
}
			   ...
			   struct spdk_json_write_ctx *w)
{
	...
	if (bsmsession == NULL || bsmsession->bdev == NULL || bsmsession->need_write_config != true) {
		return;
	}
	...
	if (bsmsession->smsession.gfunc_id > SSAM_PF_MAX_NUM || (ssam_get_en_hpd() == true)) {
		spdk_json_write_named_int32(w, "vqueue", (int32_t)bsmsession->smsession.max_queues);
	}
...
static void
ssam_blk_print_iostat_json(struct spdk_ssam_session *smsession, uint16_t vq_idx,
			   struct spdk_bdev_io_stat *stat,
			   struct ssam_blk_stat *blk_stat, struct spdk_json_write_ctx *w)
{
	...
	if (vq_idx != SPDK_INVALID_ID) {
		spdk_json_write_named_uint32(w, "vq_idx", vq_idx);
		spdk_json_write_named_uint32(w, "tid", smsession->smdev->tid);
	}
...
static void
ssam_blk_function_iostat_sum(uint16_t gfunc_id, struct spdk_bdev_io_stat *stat,
			     struct ssam_blk_stat *blk_stat)
{
	...
	while (smdev != NULL) {
		if (smdev->smsessions[gfunc_id] != NULL) {
			bsmsession = ssam_to_blk_session(smdev->smsessions[gfunc_id]);
			stat->bytes_read += bsmsession->stat.bytes_read;
			stat->num_read_ops += bsmsession->stat.num_read_ops;
			stat->bytes_written += bsmsession->stat.bytes_written;
			stat->num_write_ops += bsmsession->stat.num_write_ops;
			stat->bytes_unmapped += bsmsession->stat.bytes_unmapped;
			stat->num_unmap_ops += bsmsession->stat.num_unmap_ops;
			stat->bytes_copied += bsmsession->stat.bytes_copied;
			stat->num_copy_ops += bsmsession->stat.num_copy_ops;
			stat->read_latency_ticks += bsmsession->stat.read_latency_ticks;
			stat->max_read_latency_ticks += bsmsession->stat.max_read_latency_ticks;
			stat->min_read_latency_ticks += bsmsession->stat.min_read_latency_ticks;
			stat->write_latency_ticks += bsmsession->stat.write_latency_ticks;
			stat->max_write_latency_ticks += bsmsession->stat.max_write_latency_ticks;
			stat->min_write_latency_ticks += bsmsession->stat.min_write_latency_ticks;
			stat->unmap_latency_ticks += bsmsession->stat.unmap_latency_ticks;
			stat->max_unmap_latency_ticks += bsmsession->stat.max_unmap_latency_ticks;
			stat->min_unmap_latency_ticks += bsmsession->stat.min_unmap_latency_ticks;
			stat->copy_latency_ticks += bsmsession->stat.copy_latency_ticks;
			stat->max_copy_latency_ticks += bsmsession->stat.max_copy_latency_ticks;
			stat->min_copy_latency_ticks += bsmsession->stat.min_copy_latency_ticks;
			stat->ticks_rate += bsmsession->stat.ticks_rate;

			blk_stat->count += bsmsession->blk_stat.count;
			blk_stat->start_count += bsmsession->blk_stat.start_count;
			blk_stat->total_tsc += bsmsession->blk_stat.total_tsc;
			blk_stat->dma_tsc += bsmsession->blk_stat.dma_tsc;
			blk_stat->dma_count += bsmsession->blk_stat.dma_count;
			blk_stat->dma_complete_count += bsmsession->blk_stat.dma_complete_count;
			blk_stat->bdev_tsc += bsmsession->blk_stat.bdev_tsc;
			blk_stat->bdev_submit_tsc += bsmsession->blk_stat.bdev_submit_tsc;
			blk_stat->bdev_count += bsmsession->blk_stat.bdev_count;
			blk_stat->bdev_complete_count += bsmsession->blk_stat.bdev_complete_count;
			blk_stat->complete_tsc += bsmsession->blk_stat.complete_tsc;
			blk_stat->internel_tsc += bsmsession->blk_stat.internel_tsc;
			blk_stat->complete_read_ios += bsmsession->blk_stat.complete_read_ios;
			blk_stat->err_read_ios += bsmsession->blk_stat.err_read_ios;
			blk_stat->complete_write_ios += bsmsession->blk_stat.complete_write_ios;
			blk_stat->err_write_ios += bsmsession->blk_stat.err_write_ios;
			blk_stat->flush_ios += bsmsession->blk_stat.flush_ios;
			blk_stat->complete_flush_ios += bsmsession->blk_stat.complete_flush_ios;
			blk_stat->err_flush_ios += bsmsession->blk_stat.err_flush_ios;
			blk_stat->other_ios += bsmsession->blk_stat.other_ios;
			blk_stat->complete_other_ios += bsmsession->blk_stat.complete_other_ios;
			blk_stat->err_other_ios += bsmsession->blk_stat.err_other_ios;
			blk_stat->fatal_ios += bsmsession->blk_stat.fatal_ios;
			blk_stat->io_retry += bsmsession->blk_stat.io_retry;
		}
...
static void
ssam_blk_show_iostat_json(struct spdk_ssam_session *smsession,
			  struct spdk_ssam_show_iostat_args *args,
			  struct spdk_json_write_ctx *w)
{
	struct spdk_ssam_blk_session *bsmsession = ssam_to_blk_session(smsession);
	struct spdk_bdev_io_stat stat = {0};
	struct ssam_blk_stat blk_stat = {0};

	switch (args->mode) {
	case SSAM_IOSTAT_NORMAL:
		...
		ssam_blk_print_iostat_json(smsession, args->id, &stat, &blk_stat, w);
		break;
	case SSAM_IOSTAT_SUM:
		if (bsmsession->need_write_config == false) {
			return;
		}
		ssam_blk_function_iostat_sum(smsession->gfunc_id, &stat, &blk_stat);
		ssam_blk_print_iostat_json(smsession, args->id, &stat, &blk_stat, w);
		break;
	case SSAM_IOSTAT_DUMP_VQ:
		for (int i = 0; i < smsession->max_queues; i++) {
			if (bsmsession->vq_blk_stat[i].start_count == 0) {
				continue;
			}
			ssam_blk_print_iostat_json(smsession, i, &bsmsession->vq_stat[i], &bsmsession->vq_blk_stat[i], w);
		}
		break;
	case SSAM_IOSTAT_SPARSE:
		if (args->id == SPDK_INVALID_ID) {
			memcpy(&stat, &bsmsession->stat, sizeof(struct spdk_bdev_io_stat));
			memcpy(&blk_stat, &bsmsession->blk_stat, sizeof(struct ssam_blk_stat));
		} else {
...
ssam_blk_clear_iostat_json(struct spdk_ssam_session *smsession)
{
	...
	for (int i = 0; i < smsession->max_queues; i++) {
		memset(&bsmsession->vq_stat[i], 0,
		       sizeof(struct spdk_bdev_io_stat) - sizeof(uint64_t));  /* exclude ticks_rate */
		memset(&bsmsession->vq_blk_stat[i], 0, sizeof(struct ssam_blk_stat));
	}
}

static struct spdk_bdev *
ssam_blk_get_bdev(struct spdk_ssam_session *smsession, uint32_t id)
{
	struct spdk_bdev *bdev = ssam_get_session_bdev(smsession);
...
ssam_session_bdev_remove_cb(struct spdk_ssam_session *smsession, void **ctx)
{
	struct spdk_ssam_blk_session *bsmsession = ssam_to_blk_session(smsession);
	int rc = 0;
	int status = spdk_ssam_get_hot_upgrade_status();
	...
	if ((ssam_get_en_hpd() == true) &&
	    (g_hpd_delete_session_times[smsession->gfunc_id] == ssam_get_core_num()) && status == SSAM_HOT_UPGRADE_DONE) {
		rc = ssam_hotplug_del(smsession->gfunc_id);
		if (rc != 0) {
			SPDK_ERRLOG("BLK controller %s hotplug deleted failed\n", smsession->name);
		}
	}
	bsmsession->stop_bdev_poller = SPDK_POLLER_REGISTER(ssam_destroy_poller_cb,
				       bsmsession, 0);
	if (g_hpd_delete_session_times[smsession->gfunc_id] == ssam_get_core_num() && status == SSAM_HOT_UPGRADE_DONE) {
		rc = ssam_virtio_blk_resize(smsession->gfunc_id, 0);
		if (rc != 0) {
			SPDK_WARNLOG("%s: virtio blk resize failed when remove session.\n", smsession->name);
		}
...
static int
ssam_alloc_task_pool(struct spdk_ssam_session *smsession, uint16_t vq_idx)
{
	struct spdk_ssam_blk_session *bsmsession = ssam_to_blk_session(smsession);
	...
	if (vq->tasks == NULL || vq->index == NULL) {
		SPDK_ERRLOG("%s: failed to allocate %"PRIu32" tasks for virtqueue %"PRIu16"\n",
			    smsession->name, task_cnt, vq_idx);
		if (vq->tasks != NULL) {
			spdk_free(vq->tasks);
			vq->tasks = NULL;
		}
	...
	for (i = 0; i < task_cnt; i++) {
		task = &((struct spdk_ssam_blk_task *)vq->tasks)[i];
...
ssam_task_complete_cb(void *arg)
{
	...
	ssam_blk_stat_statistics(task, &bsmsession->stat, &bsmsession->blk_stat, cb_arg->status);
	ssam_blk_stat_statistics(task, &bsmsession->vq_stat[task->vq_idx],
				 &bsmsession->vq_blk_stat[task->vq_idx],
				 cb_arg->status);
...
ssam_task_complete(struct spdk_ssam_blk_task *task, uint8_t status)
{
	...
	ssam_task_stat_tick(&task->task_stat.complete_end_tsc);
	ssam_blk_stat_statistics(task, &bsmsession->stat, &bsmsession->blk_stat, status);
	ssam_blk_stat_statistics(task, &bsmsession->vq_stat[task->vq_idx],
				 &bsmsession->vq_blk_stat[task->vq_idx],
				 status);
	ssam_blk_task_finish(task);
		     ...
		     struct spdk_ssam_blk_task *task, uint32_t type, uint8_t status)
{
	...
	task->bsmsession->vq_blk_stat[task->vq_idx].dma_count++;
...
ssam_blk_req_complete_cb(struct spdk_bdev_io *bdev_io, bool success, void *cb_arg)
{
	...
	task->bsmsession->vq_blk_stat[task->vq_idx].bdev_complete_count++;
...
ssam_io_process(struct spdk_ssam_blk_task *task, const struct virtio_blk_outhdr *req)
{
	...
	task->bsmsession->vq_blk_stat[task->vq_idx].bdev_count++;
	uint64_t response_start = (uint64_t)time(NULL);
	...
	case VIRTIO_BLK_T_OUT:
		...
		response_end = (uint64_t)time(NULL);
		used_time = response_end - response_start;
		if (used_time > SSAM_IO_PROCESS_TIME_OUT) {
			SPDK_WARNLOG("Used too mush tine for IO(type = %u) process, time used = %u\n", req->type, used_time);
		}
		break;
	case VIRTIO_BLK_T_DISCARD:
		      ...
		      uint16_t vq_idx, uint16_t req_idx, uint32_t payload_size)
{
	...
	rc = ssam_alloc_task_pool(smsession, vq_idx);
	if (rc != 0) {
...
ssam_process_vq(struct spdk_ssam_session *smsession, struct ssam_request *io_req)
{
	...
	bsmsession->vq_blk_stat[vq_idx].start_count++;
...
ssam_blk_response_worker(struct spdk_ssam_session *smsession, void *arg)
{
	...
	if (smsession->virtqueue[vq_idx].tasks == NULL) {
		smsession->smdev->discard_io_num++;
		SPDK_ERRLOG("vq_idx invalid, vq %u has not received io\n", vq_idx);
		return;
	}
	...
	task->bsmsession->vq_blk_stat[task->vq_idx].dma_complete_count++;
...
static void
ssam_blk_construct_roll_back(uint16_t gfunc_id)
{
	struct spdk_ssam_dev *smdev = NULL;
	...
	if (ssam_get_hash_mode() == SSAM_VQ_HASH_MODE) {
		rc = ssam_virtio_vq_unbind_core(gfunc_id);
		if (rc != 0) {
			SPDK_WARNLOG("virtio blk vq unbind core failed.\n");
		}
	}

	if ((gfunc_id > SSAM_PF_MAX_NUM) || (ssam_get_en_hpd() == true)) {
		rc = ssam_virtio_blk_release_resource(gfunc_id);
		if (rc != 0) {
	...
	while (smdev != NULL) {
		...
		if ((smsession != NULL) && (smsession->smdev != NULL)) {
			ssam_to_blk_session(smsession)->stop_session_poller = SPDK_POLLER_REGISTER(ssam_destroy_session_cb,
					smsession, 0);
		}
		smdev = ssam_dev_next(smdev);
	}
...
ssam_blk_start_post_cb(struct spdk_ssam_session *smsession, void **arg)
{
	...
	g_blk_set_times[smsession->gfunc_id]++;
	if (g_blk_set_times[smsession->gfunc_id] == ssam_get_core_num()) {
		if ((ssam_get_en_hpd() == true) && (g_hpd_to_async[smsession->gfunc_id] == false)) {
			rc = ssam_hotplug_add(smsession->gfunc_id);
			if (rc != 0) {
				SPDK_ERRLOG("ssam hotplug add function(%d) config failed:%s\n", smsession->gfunc_id, spdk_strerror(-rc));
				g_hpd_add_fail_flag[smsession->gfunc_id] = true;
				ssam_blk_construct_roll_back(smsession->gfunc_id);
				goto out;
			}
		}
...
out:
	if (smsession->rsp_fn != NULL) {
		smsession->rsp_fn(smsession->rsp_ctx, rc);
		smsession->rsp_fn = NULL;
	}
...
static void
ssam_blk_del_recover(struct spdk_ssam_session *smsession, struct spdk_bdev *bdev_roback)
{
	struct spdk_ssam_session_reg_info info = {0};
	const char *dev_name = NULL;
	struct spdk_ssam_blk_session *bsmsession = ssam_to_blk_session(smsession);
	bool readonly = bsmsession->readonly;
	char *serial = bsmsession->serial;

	info.gfunc_id = smsession->gfunc_id;
	dev_name = bdev_roback->name;
	info.tid = smsession->smdev->tid;
	info.queues = smsession->max_queues;
	info.rsp_ctx = NULL;
	info.rsp_fn = NULL;

	g_hpd_to_async[smsession->gfunc_id] = true;
	ssam_free_session(smsession);
	ssam_blk_roback(&info, dev_name, readonly, serial);
	return;
}
...
static int
ssam_destroy_session_cb(void *arg)
{
	struct spdk_ssam_blk_session *bsmsession = (struct spdk_ssam_blk_session *)arg;
	struct spdk_ssam_session *smsession = &bsmsession->smsession;
	int    rc = 0;
	...
	/* stop poller */
	spdk_poller_unregister(&bsmsession->stop_session_poller);

	ssam_session_unreg_response_cb(smsession);
	rc = ssam_session_unregister(smsession, false);
	if (rc != 0) {
		SPDK_ERRLOG("function id %d: blk construct failed and session remove failed\n",
			    smsession->gfunc_id);
	}
...
static void
ssam_blk_roback(struct spdk_ssam_session_reg_info *info, const char *dev_name,
		bool readonly, char *serial)
{
	struct spdk_ssam_session *smsession = NULL;
	struct spdk_ssam_session *smsession_tmp = NULL;
	struct spdk_ssam_blk_session *bsmsession = NULL;
	struct spdk_bdev *bdev = NULL;
	...
	g_gfunc_session_number = 0;
	g_blk_set_times[info->gfunc_id] = 0;
	g_hpd_delete_session_times[info->gfunc_id] = 0;
	g_hpd_del_flag[info->gfunc_id] = 0;
	g_hpd_add_fail_flag[info->gfunc_id] = false;

	smdev = ssam_dev_next(NULL);
	while (smdev != NULL) {
		smsession_tmp = smdev->smsessions[info->gfunc_id];
		if (smsession_tmp != NULL) {
			smdev = ssam_dev_next(smdev);
			continue;
		}
		tid = smdev->tid;
		...
		bsmsession->need_write_config = ((tid == ssam_get_core_num() - 1) ? true : false);
...
out:
	smdev = ssam_dev_next(NULL);
	while (smdev != NULL) {
		smsession = smdev->smsessions[info->gfunc_id];
		   ...
		   bool readonly, char *serial)
{
	...
	g_hpd_del_flag[info->gfunc_id] = 0;
	g_hpd_add_fail_flag[info->gfunc_id] = false;
	g_hpd_to_async[info->gfunc_id] = false;

	for (int i = 0; i < ssam_get_core_num(); i++) {
		tid = i;
		if (tid == SPDK_INVALID_TID) {
			ret = -EINVAL;
			goto out;
