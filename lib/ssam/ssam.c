...
static ssam_fini_cb g_ssam_fini_cpl_cb;

static bool g_io_crc_check = false;

static int ssam_init(void);

void
spdk_ssam_set_io_crc_check(bool crc_check)
{
	g_io_crc_check = crc_check;
}

void
ssam_io_crc_check_log(const char *context, ssam_io_crc_check_cb crc_check_cb, void *arg)
{
	if (spdk_unlikely(g_io_crc_check)) {
		crc_check_cb(context, arg);
	}
}
...
{
	const struct spdk_ssam_dev_info *dev1 = (const struct spdk_ssam_dev_info *)a;
	const struct spdk_ssam_dev_info *dev2 = (const struct spdk_ssam_dev_info *)b;
	return dev1->active_session_num - dev2->active_session_num;
}

static uint32_t
ssam_get_min_payload_tids(uint16_t cpu_num, uint16_t thread_num)
{
	int real_thread_num = thread_num < cpu_num ? thread_num : cpu_num;
	struct spdk_ssam_dev *smdev = NULL;
	struct spdk_ssam_dev *tmp = NULL;
	/* All tid have been used, find a tid which has minimum device */
	struct spdk_ssam_dev_info smdevs[cpu_num];
	int count = 0;
	uint32_t mask = 0;

	TAILQ_FOREACH_SAFE(smdev, &g_ssam_devices, tailq, tmp) {
		smdevs[count].active_session_num = smdev->active_session_num;
		smdevs[count].tids = smdev->tid;
		count++;
	}

	qsort(smdevs, cpu_num, sizeof(struct spdk_ssam_dev_info), compare);

	for (uint16_t i = 0; i < real_thread_num; i++) {
		mask |= (1U << smdevs[i].tids);
	}
...
uint32_t
ssam_get_tids(uint16_t thread_num)
{
	uint32_t cpu_num;

	cpu_num = ssam_get_core_num();
	if ((cpu_num == 0) || (cpu_num > SSAM_CPU_NUM_MAX) || (thread_num == 0)) {
		SPDK_ERRLOG("CPU num %u or thread num %u not valid.\n", cpu_num, thread_num);
		return SPDK_INVALID_TID;
	}

	return ssam_get_min_payload_tids((uint16_t)cpu_num, thread_num);
}
		    ...
		    struct spdk_json_write_ctx *w)
{
	...
	if (gfunc_id == UINT16_MAX) {
		...
		while (smsession != NULL) {
			if (smsession->gfunc_id == gfunc_id) {
				spdk_json_write_object_begin(w);
				spdk_json_write_named_string(w, "ctrlr", ssam_dev_get_name(smdev));
				spdk_json_write_named_string_fmt(w, "cpumask", "0x%s",
								 spdk_cpuset_fmt(spdk_thread_get_cpumask(smdev->thread)));
...
ssam_dev_create_register(struct spdk_ssam_dev *smdev, uint16_t tid)
{
	...
	smdev->lcore_id = ssam_tid_to_cpumask(tid, &cpumask);
	if (smdev->lcore_id == SPDK_CPUSET_SIZE) {
...
static int
ssam_dev_io_complete(struct spdk_ssam_dev *smdev, struct ssam_request *io_req,
				 bool success)
{
	...
	if ((smdev->type & (VIRTIO_TYPE_SCSI | VIRTIO_TYPE_FS)) != 0 && smdev->active_session_num > 0) {
		for (int i = 0; i < SSAM_MAX_SESSION_PER_DEV; i++) {
			if (smdev->smsessions[i] != NULL) {
				smsession = smdev->smsessions[i];
				if (smsession->backend->remove_self != NULL) {
...
void
spdk_ssam_subsystem_fini(ssam_fini_cb fini_cb)
{
...
void
spdk_ssam_subsystem_init(ssam_init_cb init_cb)
{
	uint32_t i;
	int ret;
	int shm_id;
