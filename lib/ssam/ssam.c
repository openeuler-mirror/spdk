...
#define DEVICE_READY_TIMEOUT        150
#define DEVICE_READY_WAIT_TIME      10000
...
int
ssam_session_unregister(struct spdk_ssam_session *smsession, bool blk_force_delete)
{
	int rc;
...
ssam_dev_response_worker(void *arg)
{
	...
	do {
		...
		if (io_num == 0) {
			smdev->dma_response_num = dma_response_num;
		}
...
int
spdk_ssam_io_poller_start(void)
{
	int rc = 0;
	/* start io resend */
	rc = ssam_get_hot_upgrade_state();
	if (rc != 0) {
		SPDK_ERRLOG("start io resend error, rc = %d.\n", rc);
		return rc;
	}
	/* register io poller */
...
int
ssam_check_device_status(uint8_t role)
{
	uint8_t ready = 0;
	int times = 0;
	int rc;
	...
	do {
		rc = ssam_check_device_ready(role, 0, &ready);
		if (rc != 0) {
...
ssam_init(void)
{
	...
	if (spdk_ssam_get_hot_upgrade_status() != SSAM_HOT_UPGRADE_BEGIN) {
		rc = ssam_check_device_status(SSAM_NORMAL_PROCESS);
		if (rc != 0) {
			return rc;
		}
	}
	...
	ssam_init_qos();
	SPDK_NOTICELOG("ssam init completed %d.\n", rc);
	return rc;
}
